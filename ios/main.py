...
def _is_ios_runtime():
    return IS_IOS or "IOS_ARGUMENT" in os.environ
...
def download_media(
    ...
    # If ffmpeg is not available on iOS, use simplified download
    if _is_ios_runtime() and not ffmpeg_path:
        print("FFmpeg not available, using simplified download")
        return download_media_simple(url, media_type, output_path, resolution, audio_format, audio_quality, progress_hook, cancel_check)
...
class YTDownloaderLayout(BoxLayout):
    ...
    def _sec_type(self):
        ...
        type_box.add_widget(make_seg_button("Video", "mediatype", height=46))
        type_box.add_widget(make_seg_button("Audio", "mediatype", height=46))
        
        # Set default selection
        type_box.children[1].state = "down"  # Audio button (reversed order)
        
        def on_type_change(btn):
            if btn.state == "down":
                self._media_type = btn.text.lower()
                print(f"Media type changed to: {self._media_type}")
        
        for btn in type_box.children:
            btn.bind(state=on_type_change)
    ...
    def _sec_options(self):
        ...
        # Resolution spinner
        self._add(make_label("Video Resolution", font_size=9, color=MUTED))
        self._res_spinner = make_spinner(VIDEO_RESOLUTIONS, "720p")
        self._add(self._res_spinner)
        
        # Audio format
        self._add(make_label("Audio Format", font_size=9, color=MUTED))
        self._fmt_spinner = make_spinner(AUDIO_FORMATS, "mp3")
        self._add(self._fmt_spinner)
        
        # Audio quality
        self._add(make_label("Audio Quality", font_size=9, color=MUTED))
        self._qual_spinner = make_spinner(AUDIO_QUALITIES, "192 kbps")
        self._add(self._qual_spinner)
    ...
    def _sec_download(self):
        self._download_btn = make_accent_btn("Download Now", height=54, font_size=15)
        self._download_btn.bind(on_release=self._on_download)
        self._add(self._download_btn)
        
        self._cancel_btn = make_button("Cancel", bg=BORDER, fg=TEXT, height=44, font_size=12)
        self._cancel_btn.disabled = True
        self._cancel_btn.bind(on_release=self._on_cancel)
        self._add(self._cancel_btn)
                ...
                if d['status'] == 'downloading':
                    ...
                    Clock.schedule_once(
                        lambda dt: self._update_progress(
                            progress,
                            f"Downloading: {d.get('_filename', 'Unknown')} - {d.get('_percent_str', '0%')}"
                        )
                    )
                elif d['status'] == 'finished':
                    Clock.schedule_once(
                        lambda dt: self._update_progress(100, "Processing...")
                    )
    ...
    def _download_complete(self, error):
        ...
        if error:
            ...
            self._progress_label.text = "Download failed"
        else:
            show_popup("Success", f"Download complete! Saved to {OUT_DIR}")
